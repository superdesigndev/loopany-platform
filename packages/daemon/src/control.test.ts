 ...
 * (pidfile read, liveness probe, kill, server fetch, output) so nothing reads a
 * real ~/.loopany, signals a real process, or hits the network.
 */
...
describe("runStatus", () => {
  test("daemon running → reports pid", async () => {
    const cap = capture({ readPid: () => 4242, alive: () => true, server: "", token: undefined });
