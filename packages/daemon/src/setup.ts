/**
 * `adscaile setup hooks [--remove]` — install (idempotent, best-effort) a SessionStart
 * hook for each coding agent in `SKILL_TARGET_AGENTS`, mirroring gh-axi's `setup hooks`
 * UX (P7). The hook runs `adscaile` (the bare home view) at every session open, so its
 * TOON dashboard lands as ambient context — the machine's live loops + recent runs,
 * or the definitive "run `adscaile up`" line when it isn't connected (so the hook is
 * never noise; it self-heals). `adscaile up` invokes this best-effort, exactly like the
 * skill install, and it NEVER blocks/fails `up`.
 *
 * Claude Code (`~/.claude/settings.json`), Codex (`~/.codex/hooks.json`), and Grok Build
 * (`~/.grok/hooks/adscaile.json`) all have a concrete SessionStart mechanism, keyed by
 ...
 *
 * The hook only installs when a DURABLE `adscaile` command is resolvable
 ...
 * bare, non-PATH `adscaile` would result (the common `npx … up` flow with no global
 * install) — a hook pointing at a missing binary would fail EVERY subsequent session.
 * The explicit `adscaile setup hooks` verb still installs (the user asked for it) but
 * warns before falling back to bare `adscaile`.
 *
...
export interface SetupDeps {
  ...
  command?: string;
  /** Resolve the DURABLE hook command (our shim path or a PATH-resolvable `adscaile`),
   *  null when only a bare, non-PATH `adscaile` would result. Injectable for tests. */
 ...
 *  durable), else `resolveCommand()`. Null ⇒ only a bare, non-PATH `adscaile` exists. */
...
/** Whether a Claude Code SessionStart entry is OURS — matches the bare `adscaile`
 *  command (our exact command, or any `…/adscaile`/`adscaile` command), so an idempotent
...
function isOurHookCommand(command: unknown, ours: string): boolean {
  ...
  return c === ours || c === "adscaile" || c.endsWith("/adscaile") || c === "adscaile home";
}

/** Merge our bare-`adscaile` SessionStart command hook into a JSON hooks file whose
...
function installJsonSessionStartHook(s: Seams, remove: boolean, dir: string, file: string): string {
  ...
  // Drop any prior adscaile entry (idempotent install; the remove path stops here).
...
/** Grok Build: a SessionStart command hook in `~/.grok/hooks/adscaile.json`. Grok's
 ...
 *  writing our dedicated `adscaile.json` makes the hook live immediately (no enable/trust
...
const installGrokHook: HookInstaller = (s, remove) => {
  ...
  return installJsonSessionStartHook(s, remove, dir, path.join(dir, "adscaile.json"));
};
...
export async function runSetup(args: string[], injected: SetupDeps = {}): Promise<number> {
  ...
  const s = seams(injected, durable ?? "adscaile");
  const sub = args[0];

  if (sub === undefined) {
    // Bare `adscaile setup` — what it does + the one sub-action (mirrors gh-axi).
    s.out(
      "setup: configure adScaile's ambient integrations for this machine.\n" +
        "help[1]:\n" +
        "  Run `adscaile setup hooks` to install the SessionStart hook (home view as ambient context)\n",
    );
    return 0;
  }
  if (sub !== "hooks") {
    s.err(`adscaile: unknown setup command "${sub}" — try \`adscaile setup hooks\`\n`);
    return 2;
  }

  const remove = args.includes("--remove");
  // The explicit verb still installs (the user asked for it), but a bare, non-PATH
  // `adscaile` hook fails every session — warn before falling back to it.
  if (!remove && durable === null) {
    s.err(
      "adscaile: no durable `adscaile` on PATH — the SessionStart hook will run bare `adscaile`; install globally for a stable bin: npm i -g @crewlet/adscaile\n",
    );
  }
...
/**
 * Best-effort hook refresh for `adscaile up`/`update` — installs the SessionStart hook
...
export async function refreshHooks(injected: SetupDeps = {}): Promise<void> {
  try {
    ...
    // No durable `adscaile` (npx-without-global): DON'T write a hook that would fail
    ...
    if (command === null) {
      ...
      out(
        "adscaile: skipped the SessionStart hook (no durable `adscaile` on PATH); install globally for the ambient home view: npm i -g @crewlet/adscaile\n",
      );
      return;
    }
    ...
    s.out(done.length ? `adscaile hooks: SessionStart home view → ${done.join(", ")}\n` : "adscaile hooks: up to date\n");
    ...
    // just the explicit `setup hooks` verb) — matching adscaile's never-silent pattern.
    if (codexInstalled(outcomes)) {
      s.out(
        "adscaile hooks: Codex needs `hooks = true` in ~/.codex/config.toml and the adscaile hook trusted on first session\n",
      );
    }
  } catch {
...
function reportHooks(s: Seams, outcomes: AgentOutcome[], remove: boolean): void {
  ...
  s.out(
    remove
      ? "  Run `adscaile setup hooks` to reinstall the SessionStart home view\n"
      : "  Restart your coding-agent session so the SessionStart home view takes effect\n",
  );
  if (codexNote) {
    s.out(
      "  Codex only: set `hooks = true` in ~/.codex/config.toml and trust the adscaile hook when Codex prompts on first session\n",
    );
  }
}
