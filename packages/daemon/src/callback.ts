/**
 * Callback mode — `adscaile <verb> [...flags]` invoked by claude (via the PATH
...
export async function runCallback(argv: string[]): Promise<number> {
  ...
  if (r.kind === "not-configured") {
    process.stderr.write("adscaile: control channel not configured\n");
    return 2;
  }
  if (r.kind === "read-error") {
    process.stderr.write(`adscaile: cannot read ${r.path}\n`);
    return 1;
  }
  if (r.kind === "network-error") {
    process.stderr.write(`adscaile: ${r.message}\n`);
    return 1;
  }
