 ...
 *
 * The integration test spawns a FAKE `claude` (ADSCAILE_CLAUDE_BIN) that captures the
 * task it was handed, and a FAILING MCP bridge fixture (ADSCAILE_MCP_BRIDGE) so a
...
describe("buildWorkflowFallbackTask", () => {
  ...
  test("names a dated setup file and a copy-paste fix prompt for config-needed fixes", () => {
    ...
    expect(task).toContain("fix workflow issue in adscaile/cookie-report/workflow-setup-2026-07-01.md");
  });
  test("explains the allowlisted env + the ADSCAILE_WORKFLOW_ENV pass-through knob", () => {
    ...
    expect(task).toContain("ADSCAILE_WORKFLOW_ENV");
  });
  ...
  test("composes with the server-sent STATIC trigger verbatim (task column removed → d.task IS the trigger)", () => {
    ...
    const trigger =
      "[loop run · MRR loop]\n\nRun now. Read your task file at adscaile/mrr/README.md, follow its `## Spec`.\nGoal (finish line): reach 100 paying users\nEnd the run with `adscaile report` — or `adscaile finish`.";
    const t = buildWorkflowFallbackTask(trigger, failure, "2026-07-01", "mrr");
    expect(t).toContain("adscaile/mrr/README.md");
...
describe("buildWorkflowFallbackTask — SyntaxError (deterministic parse failure) branch", () => {
  ...
  test("surfaces an owner prompt carrying the loop id to edit or clear the workflow", () => {
    expect(task).toContain("adscaile edit loop-abc --workflow-file");
  ...
  test("falls back to a placeholder loop id when none is passed", () => {
    ...
    expect(t).toContain("adscaile edit <loop-id> --workflow-file");
  });
});
...
describe("buildResumeTask", () => {
  test("names the interruption, trusts prior progress, re-pins the one-report contract", () => {
    ...
    expect(t).toContain("exactly ONE `adscaile report");
  });
});

describe("buildAgentSpawn", () => {
  afterEach(() => {
    delete process.env.ADSCAILE_CLAUDE_BIN;
    delete process.env.ADSCAILE_GROK_BIN;
    delete process.env.ADSCAILE_CODEX_BIN;
  });
  ...
  test("claude-code: ADSCAILE_CLAUDE_BIN escape hatch + resume + model, sys file omitted when absent", () => {
    process.env.ADSCAILE_CLAUDE_BIN = "/opt/claude";
  ...
  test("grok: ADSCAILE_GROK_BIN escape hatch + resume + model", () => {
    process.env.ADSCAILE_GROK_BIN = "/opt/grok";
  ...
  test("codex: ADSCAILE_CODEX_BIN escape hatch + exec resume + model", () => {
    process.env.ADSCAILE_CODEX_BIN = "/opt/codex";
...
beforeEach(() => {
  root = fs.mkdtempSync(path.join(os.tmpdir(), "adscaile-runner-"));
...
describe("runDelivery — workflow failure falls back to the agent", () => {
  test("a failing tools.call routes to claude with the fallback task (not a failed run)", async () => {
    process.env.ADSCAILE_CLAUDE_BIN = writeFakeClaude();
    process.env.ADSCAILE_MCP_BRIDGE = writeFailingBridge();
    ...
    expect(captured).toContain("fix workflow issue in adscaile/cookie-report/");
  }, 20000);
...
describe("runDelivery — a timed-out run keeps its session pointer", () => {
  test("sessionId is reported even when claude times out (transcript recovery stays possible)", async () => {
    // Fresh runner import so a tiny ADSCAILE_EXEC_TIMEOUT_MS (read at module load) applies.
    vi.resetModules();
    process.env.ADSCAILE_EXEC_TIMEOUT_MS = "1500";
    ...
    process.env.ADSCAILE_CLAUDE_BIN = bin;
    ...
    try {
      ...
      srv.close();
      delete process.env.ADSCAILE_CLAUDE_BIN;
      delete process.env.ADSCAILE_EXEC_TIMEOUT_MS;
    }
...
describe("runDelivery — the exec timeout is opt-in (unlimited by default)", () => {
  test("with ADSCAILE_EXEC_TIMEOUT_MS unset, no timer is armed — a slow claude completes ok", async () => {
...
describe("runDelivery — transient failure resumes the session (bounded retry)", () => {
  test("an API-error crash retries with --resume, sums the spend, and reports one success", async () => {
    // Fresh import so a tiny ADSCAILE_TRANSIENT_RETRY_BASE_MS (module-load const) applies.
    vi.resetModules();
    process.env.ADSCAILE_TRANSIENT_RETRY_BASE_MS = "10";
    const { runDelivery: run } = await import("./runner.js");
    ...
    try {
      ...
      srv.close();
      delete process.env.ADSCAILE_TRANSIENT_RETRY_BASE_MS;
    }
...
describe("runDelivery — the system prompt file is skipped when empty (batches 1-2)", () => {
  test("an EMPTY systemPrompt → no sys file, no --append-system-prompt-file flag", async () => {
    process.env.ADSCAILE_CLAUDE_BIN = writeArgvClaude();
    const runsDir = path.join(process.env.ADSCAILE_HOME || path.join(os.homedir(), ".adscaile"), "runs");
...
describe("runDelivery — the local ADSCAILE_ROOTS jail always applies", () => {
...
describe("runDelivery — a grok loop RUNS and reports ok despite degraded telemetry", () => {
  test("grok emits its native thought/text/end stream (no Claude result event); run marked ok, no crash on missing session/cost", async () => {
    ...
    fs.chmodSync(bin, 0o755);
    process.env.ADSCAILE_GROK_BIN = bin;
