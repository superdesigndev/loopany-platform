...
import { getAuthState, getDefaultTeamRoute } from '../server/loopApi'
 ...
 * this is a thin redirect to the explicit team URL (`/t/<slug>/timeline`) so the
 ...
 * needs a `teamSlug` to build `/t/$teamSlug/timeline`, and open mode has none.
 */
export const Route = createFileRoute('/timeline')({
  ...
  loader: async (): Promise<{ mode: 'signin' | 'timeline' }> => {
    ...
    if (auth.enabled) {
      ...
      throw redirect({ to: '/t/$teamSlug/timeline', params: { teamSlug: team.slug } })
    }
