...
// The adscaile skill's reference files, inlined at build time (Vite ?raw) so they
...
// from the local install (`adscaile up` installs the skill via `npx skills` into
// ./.claude/skills/adscaile/references/), but when that install was skipped (no
...
/**
 * GET /api/skill/references/:file — serve one adscaile skill reference file. Path-safe:
