...
export const Route = createFileRoute('/api/machines/$machineId')({ server: { handlers: {
  POST: async ({ request, params }: { request: Request; params: { machineId: string } }) => {
    ...
    const body = parsed.body as { action?: unknown; teamId?: unknown; alias?: unknown; retireMachineId?: unknown }
    ...
    if (body.action === 'adopt-alias') {
      if (typeof body.teamId !== 'string' || typeof body.alias !== 'string' || typeof body.retireMachineId !== 'string') {
        return Response.json({ error: 'teamId, alias, and retireMachineId are required' }, { status: 400 })
      }
      const outcome = await store.adoptLegacyMachineAlias({
        teamId: body.teamId.trim(), newMachineId: machine.id, retireMachineId: body.retireMachineId,
        alias: body.alias, actorUserId: actor.id,
      })
      if (outcome === 'ok') return Response.json({ ok: true, alias: body.alias.trim(), retiredMachineId: body.retireMachineId })
      if (outcome === 'old-machine-online') return Response.json({ error: 'legacy machine is still online' }, { status: 409 })
      return Response.json({ error: 'not found or not eligible for legacy adoption' }, { status: outcome === 'not-found' ? 404 : 409 })
    }
