...
export const Route = createFileRoute("/api/loops/$loopId/pause")({ server: { handlers: { POST: async ({ request, params }: { request: Request; params: { loopId: string } }) => { const auth = await resolveApiContext(request, "human", true); if (!auth.ok) return authFailure(auth.error); const body = await optionalJsonBody(request); return body.ok ? apiResponse(await loopLifecycle(params.loopId, "pause", body.value, auth.context)) : body.response; } } } });
