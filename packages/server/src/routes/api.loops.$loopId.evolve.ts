...
export const Route = createFileRoute("/api/loops/$loopId/evolve")({ server: { handlers: { POST: async ({ request, params }: { request: Request; params: { loopId: string } }) => { const auth = await resolveApiContext(request, "agent", true); if (!auth.ok) return authFailure(auth.error); const raw = await rawArtifact(request); return raw.ok ? apiResponse(await replaceFromArtifact("loop", params.loopId, raw.text, auth.context, new Date(), "charter-evolved")) : raw.response; } } } });
