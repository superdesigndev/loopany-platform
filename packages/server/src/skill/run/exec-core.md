...
- **File each product as the right kind of thing.** `loopany report` above is this run's immediate channel — a short message and this run's metrics — not storage. Durable human-readable content becomes a **doc**: `loopany doc create --file <path>`, with a stable `key:` in its front matter that you rewrite in place next run (`loopany doc update <key> --file <path>`) — never a new doc per day. Something to track or revisit becomes a **task**: `loopany task create --file <path>`, with `--follow-up <date>` to be woken about it or `--needs-human "<question>"` to ask the owner. An external artifact you produced — a PR, an issue, a deploy — gets a **mirror** attached to the task or doc that owns it: `loopany mirror attach <object-id> --kind github-pr --coords <owner/repo#n>`. The loop folder holds only what the dashboard and exports need.
