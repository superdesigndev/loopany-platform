...
- **File each product as the right kind of thing — nothing on this machine reaches the server by itself.** `loopany report` above is this run's immediate channel — a short message and this run's metrics — not storage. Durable human-readable content becomes a **doc**: `loopany doc create --file <path>`, with a stable `key:` in its front matter that you rewrite in place next run (`loopany doc update <key> --file <path>`) — never a new doc per day. Something to track or revisit becomes a **task**: `loopany task create --file <path>`, with `--follow-up <date>` to be woken about it or `--needs-human "<question>"` to ask the owner. An external artifact you produced — a PR, an issue, a deploy — gets a **mirror** attached to the task or doc that owns it: `loopany mirror attach <object-id> --kind github-pr --coords <owner/repo#n>`. A file you merely write to disk is local scratch: if it is not filed through one of these verbs, nobody will ever see it.
- **Keep the loop folder tidy.** This loop's folder ({{taskFile}}'s directory) holds the task file and whatever local scratch you need; it is often a real repo. NEVER build heavy work products inside it: a repo clone, a git worktree, `node_modules`, build output, or caches. When a task needs a checkout or scratch space, do that work OUTSIDE the loop folder (e.g. a sibling dir or a temp dir like `$(mktemp -d)`) and clean up after yourself.
...
For the full run protocol — task-file `## Spec`/`## Current understanding`/`## Timeline` discipline, when to speak, the product/object model (doc, task, mirror) in depth, schedule levers (`loopany show` → `reschedule`/`set-cron`), and dashboard/front-matter conventions — use the loopany skill installed at user scope. If it is unavailable, the rules above are sufficient.
