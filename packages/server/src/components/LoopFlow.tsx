...
import { FLOWS, type FlowSpec, type NodeDef } from '../lib/templateFlow'
import { FlowDashboard } from './FlowDashboardWidgets'
...
// ── Dashboard widgets ────────────────────────────────────────────────────────
// The widget renderers live in the shared `FlowDashboardWidgets` (also drawn by the
// public template detail's dashboard preview), so the two previews cannot drift.
function Dashboard({ spec }: { spec: FlowSpec }) {
  return (
    <div className="min-w-0">
      ...
      <FlowDashboard widgets={spec.dashboard} />
    </div>
  )
}
