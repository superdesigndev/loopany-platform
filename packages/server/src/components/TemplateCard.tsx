 ...
 *
 * `composeDirect` (the teaser surfaces) points the WHOLE card at the compose flow
 * (`/?template=<name>` — DashboardView.openTemplate; preserved across the login
 * redirect for a signed-out visitor) instead of the market detail page, and drops the
 * then-redundant Create pill. The market keeps detail navigation (the default).
 */
  ...
  return (
    ...
    >
      ...
      {/* Footer: 3 compact indicators + the quiet hover-revealed Create affordance
          (omitted when the whole card already composes — it would duplicate). */}
