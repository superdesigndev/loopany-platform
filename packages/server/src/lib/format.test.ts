...
describe('cronText', () => {
  ...
  // An ABSENT cron is not an unparseable one: it means the row is an inert task
  // (tracked work, no schedule), so the label states that rather than rendering
  // blank. Empty string is treated as absent — the column is nullable, and a
  // blank cadence chip would read as a rendering bug.
  it('names an absent cron "manual" rather than rendering blank', () => {
    expect(cronText(null)).toBe('manual')
    expect(cronText('')).toBe('manual')
  })
})
