...
export const TEMPLATE_RATINGS: Record<string, TemplateRating> = {
  ...
  'seo-try-keywords': {
    ...
    mechanism: 'open',
    visibility: 'compounds',
    visibilityNote: 'Most bets are duds by design; the day-7 verdicts are the product, and the winners compound over weeks.',
    schedule: "Weekly · before your review",
    does: "Scores every open bet on the daily series → mines one new bet",
    outcome: "🔀 One seed-article PR + day-7 verdicts",
  },
  'seo-scale-keywords': {
    ease: 'advanced',
    cadence: 'short',
    mechanism: 'open',
    visibility: 'few-runs',
    visibilityNote: 'One gated PR per run; the family footprint and position hold show in the daily series over a couple of weeks.',
    schedule: "Daily (staggered off siblings)",
    does: "Reads the daily series → one regime unit: cluster article or enrichment",
    outcome: "🔀 One gated, verified PR per run",
  },
  ...
  'follow-up-tracker': {
    ...
    mechanism: 'closed',
    visibility: 'first-run',
    visibilityNote: 'Reports what it observes from the first run, then stays quiet unless something goes wrong.',
    schedule: "A few runs per day (you set it)",
    does: "Watches what you just shipped, speaks up if it misbehaves",
    outcome: "🔔 Quiet while it behaves, an alert when it does not",
  ...
  'ab-experiment-watch': {
    ...
    mechanism: 'closed',
    visibility: 'few-runs',
    visibilityNote: 'Every run posts where the metric stands; the payoff is the verdict when the window closes.',
    schedule: "Daily through the window (you set it)",
    does: "Reads one deciding metric against the threshold you set",
    outcome: "🎯 Ship it, kill it, or extend",
    exitCondition: "Finishes when the metric settles the question, or the decision window closes.",
  },
  'bug-vigil': {
    ease: 'moderate',
    cadence: 'short',
