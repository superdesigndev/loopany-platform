...
- **Verification**: `pnpm -r typecheck` green; 26 daemon + 64 server tests green; daemon build green; a real `npx skills` e2e install verified. (Note: the server nitro `build` is green - both warm and clean rebuilds locally and in CI/Fly prod deploys; an earlier report of a `tslib`/`@aws-crypto` vendor-bundling failure could not be reproduced and was an environment-specific/transient local issue, not a standing defect - the `tslib (2.8.1)` line is nitro's normal dependency-tracing output.)
