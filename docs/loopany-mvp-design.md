...
### D14 - Loopany skill as an installable agent skill + auto-install at `loopany up` (landed, new feature not in the original plan)

- **Motivation**: the loop-builder knowledge lived as ONE inline doc (`packages/server/src/SKILL.md`) served only over HTTP at `/api/skill`. Two problems: ① a single growing doc is harder for an agent to navigate than a routing overview + focused references; ② the agent had to fetch it over the network every capture, with no local, persistent skill install. D14 turns it into a **real, installable agent skill** and **auto-installs it locally** during `loopany up`, reusing the `npx skills` CLI (vercel-labs/skills) as the installer rather than hand-rolling detect/install/no-clobber/version logic.
...
- **Verification**: `pnpm -r typecheck` green; 18 daemon + 64 server tests green; daemon build green; a real `npx skills` e2e install verified. (Note: the server nitro `build` is green - both warm and clean rebuilds locally and in CI/Fly prod deploys; an earlier report of a `tslib`/`@aws-crypto` vendor-bundling failure could not be reproduced and was an environment-specific/transient local issue, not a standing defect - the `tslib (2.8.1)` line is nitro's normal dependency-tracing output.)
