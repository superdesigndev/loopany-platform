...
- **Verification**: `pnpm -r typecheck` green; 18 daemon + 58 server tests green; daemon build green; a real `npx skills` e2e install verified. (Note: the server nitro `build` fails on a **pre-existing**, unrelated `tslib`/`@aws-crypto` vendor-bundling resolution issue - reproduced at the clean base commit with these changes stashed.)
